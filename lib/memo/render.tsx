              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {memo.generation_model ? `Generated by ${memo.generation_model.split('/')[1] || memo.generation_model}` : 'AI-generated content'}
                  </p>
                </div>
              </div>
    ...
    <div className="min-h-screen bg-[#0F172A] text-white">
      ...
      <div className="border-b-2 border-white/10 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 py-16 relative">
          ...
          {/* Author and verification byline */}
          <div className="flex flex-wrap items-center gap-4 pt-6 border-t-2 border-white/10">
            ...
            <div className="flex items-center gap-3 ml-auto">
              {memo.review_status === 'human_approved' && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-green-500/20 text-green-400 border border-green-500/30 uppercase tracking-wide">
      ...
      <main className="max-w-4xl mx-auto px-6 py-16">
        {/* Transparency statement */}
        {memo.generation_model && (
          <div className="bg-white/5 border-2 border-white/10 p-6 mb-12">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-white uppercase tracking-wide mb-2">About This Article</p>
                <p className="text-sm text-slate-400 leading-relaxed">
                  {memo.provenance?.source_competitor ? (
                    <>
                      This article was inspired by content from <strong className="text-white">{memo.provenance.source_competitor}</strong> and 
                      generated using <strong className="text-white">{brand.name}&apos;s</strong> verified brand profile to create authentic, 
                      accurate content that reflects our expertise and perspective.
                    </>
                  ) : (
                    <>
                      This article was generated using <strong className="text-white">{brand.name}&apos;s</strong> verified brand profile 
                      to create authentic, accurate content that reflects our expertise and perspective.
                    </>
                  )}
                </p>
                ...
                <details className="mt-4 text-xs text-slate-500">
                  <summary className="cursor-pointer hover:text-slate-300 font-bold uppercase tracking-wide">View full provenance</summary>
                  <div className="mt-3 p-4 bg-white/5 border border-white/10 space-y-2">
                    <div className="flex justify-between">
                      <span className="text-slate-500">AI Model</span>
                      <span className="font-mono text-slate-300">{memo.generation_model}</span>
                    </div>
                    {memo.generation_duration_ms && (
                      <div className="flex justify-between">
                        <span className="text-slate-500">Generation time</span>
                        <span className="text-slate-300">{(memo.generation_duration_ms / 1000).toFixed(1)}s</span>
                      </div>
                    )}
                    {memo.generation_tokens?.total && (
                      <div className="flex justify-between">
                        <span className="text-slate-500">Tokens used</span>
                        <span className="text-slate-300">{memo.generation_tokens.total.toLocaleString()}</span>
                      </div>
                    )}
                    {memo.provenance?.source_url && (
                      <div className="flex justify-between">
                        <span className="text-slate-500">Inspired by</span>
                        <a href={memo.provenance.source_url} target="_blank" rel="noopener noreferrer" className="text-[#0EA5E9] hover:underline truncate max-w-48">
                          {new URL(memo.provenance.source_url).hostname}
                        </a>
                      </div>
                    )}
                    {memo.provenance?.generated_at && (
                      <div className="flex justify-between">
                        <span className="text-slate-500">Generated</span>
                        <span className="text-slate-300">{new Date(memo.provenance.generated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                      </div>
                    )}
                  </div>
                </details>
