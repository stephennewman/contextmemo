...
const MEMO_TYPES = ['how_to', 'response']
const ROUTE = '/memos/how-to'
const PAGE_TITLE = 'How-To'
const PAGE_DESCRIPTION = 'Step-by-step tactical guides for AI search optimization, getting cited by ChatGPT, and improving brand visibility.'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!slugPath) {
    return {
      title: `${PAGE_TITLE} Guides - Context Memo`,
...
export default async function HowToPage({ params }: Props) {
  ...
  // Index page - show all how-to memos
  if (!slugPath) {
        ...
        <div className="border-b-2 border-white/10 relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-6 py-16 relative">
            <nav className="text-sm font-semibold text-slate-500 mb-6 uppercase tracking-wide">
              ...
              <a href="/memos" className="hover:text-white transition-colors">Memos</a>
              <span className="mx-2 text-white/20">/</span>
              <span className="text-white">{PAGE_TITLE}</span>
            </nav>
            <h1 className="text-4xl md:text-5xl font-black tracking-tight">{PAGE_TITLE} Guides</h1>
        ...
        <main className="max-w-4xl mx-auto px-6 py-12">
          ...
          {memos && memos.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white/5 border-2 border-white/10">
              ...
              <p className="text-slate-500">No how-to guides published yet.</p>
            </div>
          )}
        </main>
        ...
        <footer className="border-t-2 border-white/10 mt-auto">
          <div className="max-w-4xl mx-auto px-6 py-8">
            <div className="flex items-center justify-between text-sm">
              ...
              <a href="/memos" className="text-slate-400 hover:text-white transition-colors font-semibold uppercase text-xs tracking-wide">
                All Memos
              </a>
            </div>
          </div>
  ...
  // Try to find memo with various slug formats
  const possibleSlugs = [
    slugPath,
    `how-to/${slugPath}`,
    `resources/${slugPath}`,
